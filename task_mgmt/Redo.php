...
if ($userType !== "user") {
    $update_review = "UPDATE sub_task_mgmt SET isvarified=0, review=0, remarks='Redo', fileupload=NULL WHERE main_task_id='$main_task_id' AND project_id='$project_id' AND isvarified='1' AND review='1' AND Id='$sub_task_id'";
    $result_update = mysqli_query($conn, $update_review);
    if(!$result_update) {
        echo "Verification Failed: " . mysqli_error($conn);
    } else {
        echo "<script>alert('Redo');</script>";
